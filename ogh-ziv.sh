...
draw_vps() {
    ...
    echo ""
    echo -e "  ${A1}╔══════════════════════════════════════════════════════════╗${NC}"
    printf  "  ${A1}║${NC}  ${A4}◈${NC} ${BLD}${A4}INFO VPS${NC}  ${DIM}${now_time}  │  ${now_date}${NC}%*s${A1}║${NC}\n" 28 ""
    echo -e "  ${A1}╠══════════════════════════════════════════════════════════╣${NC}"
    ...
    # Row: CPU | RAM (tanpa DISK)
    printf  "  ${A1}║${NC}  ${DIM}CPU${NC} ${LG}%s%%${NC}  ${LG}%s${NC}  ${A1}│${NC}  ${DIM}RAM${NC} ${A3}%s/%sMB${NC}  ${A3}%s${NC}  ${A1}║${NC}\n" \
        "${cpu}" "${cpu_bar}" "${ram_u}" "${ram_t}" "${ram_bar}"

    echo -e "  ${A1}╠╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╣${NC}"

    # Row: DISK (baris tersendiri di bawah CPU/RAM)
    printf  "  ${A1}║${NC}  ${DIM}DISK${NC} ${Y}%s/%s${NC}  ${Y}%s${NC}%*s${A1}║${NC}\n" \
        "${du}" "${dt}" "${disk_bar}" 20 ""

    echo -e "  ${A1}╠══════════════════════════════════════════════════════════╣${NC}"

    # ── Status row ───────────────────────────────────────────
    printf  "  ${A1}║${NC}  ${svc_col}${svc_ic} %-8s${NC}  ${A1}│${NC}  ${DIM}AKUN${NC} ${A3}%-3s${NC}  ${A1}│${NC}  ${DIM}EXP${NC} ${LR}%-3s${NC}  ${A1}│${NC}  ${DIM}BOT${NC} ${bot_col}%-12s${NC}  ${A1}║${NC}\n" \
        "$svc_txt" "$total" "$expc" "$bot_txt"

    echo -e "  ${A1}╠╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╣${NC}"

    # Row: TEMA (baris tersendiri di bawah status)
    printf  "  ${A1}║${NC}  ${DIM}TEMA${NC}  %-55b${A1}║${NC}\n" "$tema_display"
